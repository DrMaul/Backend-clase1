...
const filtroDebug = winston.format(data => {
    // console.log(data)
    if (data.level == "debug") return data
})
...
let debug = true //args x consola o variables de entorno
if(debug == true){
...
const transportePersonalizadoConsola = new winston.transports.Console(
    {
        level:"leve",
        format: winston.format.combine(
            winston.format.colorize({
...
if (debug == true){
    loggerPersonalizado.add(transportePersonalizadoConsola)
}
