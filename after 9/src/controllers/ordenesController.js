...
export const createOrden = async (req,res)=> {
    ...
    if(!usuario){
        res.setHeader('Content-Type','application/json');
        return res.status(400).json({error:`No existe usuarios con id ${uid} en BBDD`})
    }
    ...
    if(!negocio){
        res.setHeader('Content-Type','application/json');
        return res.status(400).json({error:`No existe negocio con id ${nid} en BBDD`})
    }
    ...
    pedido.forEach(i=>{
        ...
        if(producto){
            ...
            error=true
            detalleError.push(`No existe el item ${i.id} en el negocio ${negocio.nombre}`)
        }
    })
