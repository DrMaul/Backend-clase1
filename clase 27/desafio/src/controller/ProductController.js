...
export class ProductController{
    ...
    static getProduct = async (req, res)=>{
        ...
        try {
            let product = await productDao.getProductsBy({_id:id})
            if (!product){
    ...
    static createProduct = async (req,res)=>{
        ...
        let existe
        try {
            existe = await productDao.getProductBy({code})
        } catch (error) {
            res.setHeader('Content-Type','application/json');
            return res.status(500).json(
                {
        ...
        let nuevoProducto
        try {
            nuevoProducto = await productDao.addProduct({title, description, code, price, status, stock, category, thumbnail}) 
    ...
    static updateProduct = async (req,res)=> {
        ...
        if(aModificar.code){
            let existe
            try {
                existe = await productDao.getProductsBy({_id:{$ne:id},code: aModificar.code})
                if(existe){
                    res.setHeader('Content-Type','application/json');
        ...
        try{
            let productoModificado = await productDao.updateProduct(id, aModificar)
            res.setHeader('Content-type', 'application/json')
    ...
    static deleteProduct = async (req,res)=> {
        ...
        let productos
        try{
            productos = await productDao.getProducts()
            productoEliminado = await productDao.deleteProduct(id)
